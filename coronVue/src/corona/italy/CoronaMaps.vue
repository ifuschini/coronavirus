...
import 'ammap3'
import 'ammap3/ammap/maps/js/italyLow'
...
export default {
  ...
  methods: {
    normalizeData (action = null) {
      ...
      this.arrayDatesAvailable = Object.keys(this.arrayDataNations.Lombardia)
    ...
    getDataCoorinates () {
      axios
        .get('data/commons/bubbleItalyData.json')
        .then(response => {
  ...
  created () {
    serverBus.$on('stopItalyCron', (action) => {
